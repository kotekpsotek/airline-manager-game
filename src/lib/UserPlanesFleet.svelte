<script lang="ts">
    ...
    function getAssignationRouteNameForGUI(planeId: string): string {
        ...
        return routeWhetherIs ? `${routeWhetherIs[1].from.name} - ${routeWhetherIs[1].to.name} (${routeWhetherIs[0]})` : "Not assigned"
    }
</script>
        ...
        {#if $userData}
            <table>
                ...
                {#each $userData?.fleet as fleet_unit, it_id}
                    <tr>
                        <td>{it_id + 1}</td>
                        <td>{fleet_unit.planeId}</td>
                        <td>{fleet_unit.airplane_brand} {fleet_unit.airplane_model_name}</td>
                        <td>{getAssignationRouteNameForGUI(fleet_unit.planeId)}</td>
                    </tr>
                {/each}
            </table>
        {/if}
